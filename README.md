...
多订阅聚合与 Clash / Mihomo 分流工作区。把多个订阅汇成一个固定链接，沿用 iKuuu 的 9,816 条基础规则，遇到特殊需求时再添加个人规则。

## 使用

1. 在「订阅来源」添加 Clash YAML 订阅，也可每行一个批量导入；刷新后显示节点数、流量及到期信息。
2. 在「节点与策略组」选择来源，使用手动勾选、名称正则或全部节点建立组。组内支持手选默认节点、自动测速、故障转移。
3. 新配置只使用 iKuuu 模板，不预填数据库 IP、GPT 专线或地区节点组。已保存的个人配置继续保留。
4. 「分流规则 → 我的规则」粘贴地址，选择直连、指定出口或拦截；支持备注、启用 / 暂停、上移 / 下移。网址和数据库连接地址只提取主机名，不保存凭证与路径；域名默认匹配其子域名，单个 IP 自动生成精确主机规则。复杂匹配可手动添加。
5. 「默认规则与分类」可搜索模板明细，为某条规则指定其他出口，系统会添加优先的个人规则。也可调整整个类别的候选出口及默认项。
6. 「发布与同步」生成固定订阅链接，导入 Clash。以后发布更新，客户端刷新同一链接即可。刷新会应用网页设定的默认节点；运行期间仍可在客户端临时切换。
7. 使用右上角导出配置备份。新设备可导入 JSON，或凭「配置 ID 编辑凭证」恢复云端配置。
...
- 主选择组默认保留订阅组：选择 iKuuu 等来源后，跟随该订阅组当前选中的节点。可以按需改成单个节点、个人节点组或自动测速。
- 个人规则从上到下优先匹配，然后进入 iKuuu 模板；暂停或删除覆盖规则会恢复模板行为。修改仅影响自己的聚合链接。
